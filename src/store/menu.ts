...
import { menuUser } from "@/core/data/menu-user";
...
export const useMenuStore = defineStore("menu", () => {
  ...
  let data = ref(is_admin ? menu : menuUser);
  ...
  function searchterm(terms: any) {
    ...
    data.value.filter((menuItems: any) => {
      if (
        menuItems.title?.toLowerCase().includes(terms) &&
        menuItems.type === "link"
      ) {
        items.push(menuItems);
      }
      menuItems.children?.filter((subItems: any) => {
        if (
          subItems.title?.toLowerCase().includes(terms) &&
          subItems.type === "link"
        ) {
          subItems.icon1 = menuItems.icon1;
          items.push(subItems);
