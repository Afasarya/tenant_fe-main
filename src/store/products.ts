...
export const useProductsStore = defineStore("products", () => {
  ...
  function updateCartQuantity(item: any) {
    ...
    cartItems.value.find((items: any, index: any) => {
      if (items.sku === item.product.sku) {
        ...
        if (qty !== 0 && stock) cartItems.value[index]["quantity"] = qty;
        else localStorage.setItem("cartItem", JSON.stringify(cartItems));
        return true;
      }
    });
  }
  ...
  const getBrands: any = computed(() => {
    const brands = [
      ...new Set(products.value.map((product: any) => product.brand)),
    ];
    return brands;
  });
