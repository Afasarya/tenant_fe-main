    ...
    >
      <div class="col-12">
        ...
        <BaseInput
          ...
          required
          error-message="Silahkan isi nama katgori"
...
const onBack = () => {
  router.push({ name: "KategoriProduk" });
};
...
const onCreate = async (payload: any) => {
  return await post("/productCategory/store", { ...payload });
};

const onUpdate = async (payload: any, id: any) => {
  return await put(`/productCategory/update/${id}`, { ...payload });
};
...
const onSubmit = async () => {
  ...
  if (check) {
    ...
    if (success) {
      ...
      setTimeout(() => router.push({ name: "KategoriProduk" }), 2000);
...
const fetchDetail = async () => {
  loading.value = true;
  const { data, success, message } = await get(`/productCategory/${id.value}`);
  if (success) {
