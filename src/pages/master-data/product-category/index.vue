<template>
  <div class="card container-fluid px-4 card-custom">
    <Table
      ...
      @fetch="fetchProductCategiryList()"
    />
  </div>
</template>
...
const tableConfig = reactive({
  buttonName: "Tambah Kategori Produk",
  items: [],
...
const onSearch = (query: string) => {
  ...
  fetchProductCategiryList();
};
const onAdd = () => {
  router.push({ name: "BuatKategoriProduk" });
};
const onEdit = (id: string | number) => {
  router.push(`/master/edit_kategori_produk/${id}`);
};
const onView = (id: string | number) => {
  router.push(`/master/detail_kategori_produk/${id}`);
};
const onDelete = async (id: string | number) => {
  return await remove(`/productCategory/delete/${id}`);
};
...
const fetchProductCategiryList = async () => {
  tableConfig.loading = true;
  try {
    const { data } = await get("/productCategory", { params: payload.value });
...
onMounted(() => fetchProductCategiryList());
</script>
