    ...
    >
      <div class="col-12">
        ...
        <BaseInput
          ...
          required
          error-message="Silahkan isi nama jabatan"
...
const onCreate = async (payload: any) => {
  return await post("/position/store", { ...payload });
};

const onUpdate = async (payload: any, id: any) => {
  return await put(`/position/update/${id}`, { ...payload });
};
...
const onSubmit = async () => {
  ...
  if (check) {
    ...
    if (success) {
      showSuccessToast(message ?? "Master data jabatan berhasil disimpan");
      setTimeout(() => router.push("/master/jabatan"), 2000);
...
const fetchDetail = async () => {
  loading.value = true;
  const { data, success, message } = await get(`/position/${id.value}`);
  if (success) {
