...
const onAdd = () => {
  router.push({ name: "BuatRekomemdasi" });
};
const onEdit = (id: string | number) => {
  router.push(`/rekomendasi/edit_rekomendasi/${id}`);
};
const onView = (id: string | number) => {
  router.push(`/rekomendasi/detail_rekomendasi/${id}`);
};
const onDelete = async (id: string | number) => {
  return await remove(`/Recommendation/delete/${id}`);
};
